...
class OrdersDao(object):
    ...
    def get_order_lines_by_order_id(self, order_id):
        sql = f'SELECT * FROM coolsite2.wp_woocommerce_order_items WHERE order_id = {order_id};'
        return self.db_helper.execute_select(sql)

    def get_order_items_details(self, order_item_id):
        sql = f'SELECT * FROM coolsite2.wp_woocommerce_order_itemmeta WHERE order_item_id = {order_item_id}'
