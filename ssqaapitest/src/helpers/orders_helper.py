...
class OrdersHelper(object):
    ...
    def verify_order_is_created(order_json, exp_cust_id, exp_products):
        ...
        assert order_json[
                   'customer_id'] == exp_cust_id, f"Create order with given customer_id returned bad customer id." \
                                                  f"Expected customer_id={exp_products} but got {order_json['customer_id']}"
